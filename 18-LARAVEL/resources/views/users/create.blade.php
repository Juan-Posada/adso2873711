...
@section('content')
    ...
    <main class="bg-[#154869] pt-20 bg-cover w-full min-h-[100dvh] flex flex-col justify-center items-center">
        <div class="bg-[#0006] md:w-10/12 w-full text-white p-10 rounded-lg flex flex-col justify-center items-center">
            ...
            {{-- Avatar --}}
            <form action="{{ url('users') }}" method="POST" enctype="multipart/form-data" class="my-4 flex flex-col gap-2 ">
                @csrf
                <div class="avatar mt-6 flex flex-col items-center gap-2">
                    <div id="upload" class="my-6 mask mask-squircle w-48 cursor-pointer hover:scale-105 transition-transform">
                ...
                <div class="mt-4">
                    ...
                    <input type="text" name="document" placeholder="750001002" class="w-full input bg-[transparent] border-white" value="{{ old('document') }}"/>
                </div>
                <div>
                    <label class="mt-4">Full Name:</label>
                    <input type="text" name="fullname" placeholder="John Wick" class="w-full input bg-[transparent] border-white" value="{{ old('fullname') }}"/>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="date" name="birthdate" placeholder="2000-10-08" class="w-full input bg-[transparent] border-white" value="{{ old('birthdate') }}"/>
                </div>
                <div>
                    <label class="mt-4">Phone Number:</label>
                    <input type="text" name="phone" placeholder="3201231234" class="w-full input bg-[transparent] border-white" value="{{ old('phone') }}"/>
                </div>
                <div>
                    <label class="mt-4">Email:</label>
                    <input type="text" name="email" placeholder="jonhw@mail.com" class="w-full input bg-[transparent] border-white" value="{{ old('email') }}"/>
                </div>
                <div>
                    <label class="mt-4">Password:</label>
                    <input type="password" name="password" placeholder="secret" class="w-full input bg-[transparent] border-white" />
                </div>
                <div class="mb-4">
                    <label class="mt-4">Password Confirmation:</label>
                    <input type="password" name="password_confirmation" placeholder="secret" class="w-full input bg-[transparent] border-white" />
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            @if (count($errors->all()) > 0)
                @php $content = '<ul class="flex flex-col gap-2">' @endphp
